...
class NetworkTrainer:
    ...
    async def train_async(
        ...
        if args.dataset_class is None:
            ...
            if use_user_config:
                logger.debug(f"Loading dataset config from {args.dataset_config}")
                ...
                if use_dreambooth_method:
                    logger.debug("Using DreamBooth method.")
                    user_config = {
                        "datasets": [{
                    ...
                    logger.debug("Training with captions.")
                    user_config = {
                        "datasets": [{
                            "subsets": [{
        ...
        # acceleratorを準備する
        logger.debug("preparing accelerator")
        ...
        logger.debug(f"import network module: {args.network_module}")
        ...
        if args.scale_weight_norms and not hasattr(
            ...
            logger.warning(
                "scale_weight_norms is specified but the network does not support it / scale_weight_normsが指定されていますが、ネットワークが対応していません"
            )
        ...
        if args.network_weights is not None:
            ...
            logger.debug(f"load network weights from {args.network_weights}: {info}")
        ...
        if args.full_fp16:
            ...
            logger.debug("enable full fp16 training.")
            network.to(weight_dtype)
        elif args.full_bf16:
            assert (
                args.mixed_precision == "bf16"
            ), "full_bf16 requires mixed precision='bf16' / full_bf16を使う場合はmixed_precision='bf16'を指定してください。"
            logger.debug("enable full bf16 training.")
            network.to(weight_dtype)
        ...
        if args.fp8_base:
            ...
            logger.debug("enable fp8 training.")
        ...
        logger.debug(f"  num epochs / epoch数: {num_train_epochs}")
        logger.debug(
            f"  batch size per device / バッチサイズ: {', '.join([str(d.batch_size) for d in train_dataset_group.datasets])}"
        )
        # logger.debug(f"  total train batch size (with parallel & distributed & accumulation) / 総バッチサイズ（並列学習、勾配合計含む）: {total_batch_size}")
        logger.debug(
        ...
        def save_model(
            ...
            logger.debug(f"saving checkpoint: {ckpt_file}")
        ...
        def remove_model(old_ckpt_name):
            ...
            if os.path.exists(old_ckpt_file):
                logger.debug(f"removing old checkpoint: {old_ckpt_file}")
        ...
        for epoch in range(num_train_epochs):
            logger.debug(f"epoch {epoch+1}/{num_train_epochs}")
